...
from utils import get_steem_conn, get_current_vp, url, reputation, tokenize
...
class TagBot:
    ...
    def fetch_tag(self, tag, start_author=None, start_permlink=None, posts=[], scanned_pages=0):
    ...
    def in_global_blacklist(self, author):
        '''
        @dev Check if author is in global blacklist
        @param author The username that we want to check
        @return bool True if the user is black listed globally
        '''
    ...
    def start_voting_round(self):
        '''
        @dev the main voting round script
        '''
        posts = []
        ...
        for post in posts:
            ...
            # Check if the post is downvoted by any trusted account
        ...
        for post in filtered_posts:
            ...
            if self.in_global_blacklist(post.get("author")):
                logger.info(
                    "Post is skipped since @{} is globally blacklisted".format(post.get("author"))
                    )
                continue
    ...
    def upvote(self, post, weight, retry_count=None, reply=True):
        ...
        if self.config["BOT_ACCOUNT"] in [
            ...
            logger.error(
                "Already voted on this. Skipping... {}".format(post.identifier))
            return
    ...
    def reply(self, post):
        if self.config.get("POST_REPLY_TEMPLATE"):
            ...
            logger.info(" No comment specified, skipped ")
